...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], f"train_images_{text}/%d.png" % batches_done, nrow=5, normalize=True)


z = Variable(Tensor(np.random.normal(0, 1, (100, opt.latent_dim))))
gen_imgs = generator(z)
for i in range(0, len(gen_imgs)):
    save_image(gen_imgs.data[i], f"gen_images_{text}/%d.png" % i, normalize=True)
